...
\begin{question}{1 (b)}
  ...
  \begin{lemma}{1.4} Algorithm InsertFlipSort correctly produces a sorted list \\ \\
    ...
    $A[i-(m-1)+1..k] = A[i-m+2..k]$ is sorted.  Note that $j = i - (m-1) = i-m+1$.\\
    (Induction Step) Following the $m-1th$ iteration we have,
    \begin{align*}
      & A[j] \succ A[j + 1] \land j > 0 &\\
      & \Rightarrow A[i-m+1] \succ A[i-m+2] \land i-m+1 > 0 &
    \end{align*}
  \end{lemma}
\end{question}
...
\begin{question}{3}
  ...
  \begin{lemma}{3.1} Algorithm MaximumMinimumDistance correctly produces
    ...
    It remains to show that $dist(W, \overline{W})$ is maximized over all other
    paritions of P.
